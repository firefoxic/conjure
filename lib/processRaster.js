...
export async function processRaster (options) {
	...
	for (let filePath of rasterPaths) {
		...
		let metaData = isMetaFileExists && addMetaData ? await readJsonFile(metaJsonPath) : { name: match[1] }

		if (isMetaFileExists && (metaData.maxDensity !== originDensity)) {
			info(`${UP_ONE_LINE}${UP_ONE_LINE}specified originDensity not equal to maxDensity in Metadata file "${metaJsonPath}".\nProcessing of "${filePath}" file skipped.\n\n\n`)
			continue
		}
		...
		if (isMetaFileExists && !isArraysEqualIgnoreCaseUnordered(metaData.formats, formats)) {
			info(`${UP_ONE_LINE}${UP_ONE_LINE}Metadata file "${metaJsonPath}" has different formats than specified.\nProcessing of "${filePath}" file skipped.\n\n\n`)
			continue
		}
		...
		await writeFile(metaJsPath, dests)
	}
}
