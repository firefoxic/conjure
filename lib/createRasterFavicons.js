 ...
 * @param {string[]} options.vectorPaths - Array of paths to vector icon files.
 * @param {string} options.inputDirectory - Path to the directory containing raw files.
	...
	for (const filePath of vectorPaths) {
		...
		for (const format of formats) {
			for (const size of sizes) {
				...
				icons.push({
					src: `./${normalize(`${rootRelativePath}/${baseName}-${size}.${format}`)}`,
					sizes: `${size}x${size}`,
					type: `image/${format}`,
				})
			}
