...
export async function prepareSourceFaviconFiles (options) {
	...
	const isIconTouchExists = vectorPaths.includes(pathToTouch)
	const isIcon32Exists = vectorPaths.includes(pathTo32)
	const isIcon16Exists = vectorPaths.includes(pathTo16)
	const isIconExists = vectorPaths.includes(pathToIcon)
